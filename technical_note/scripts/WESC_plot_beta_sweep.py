...
def plotting_polars_beta_correction_comparison(
    ...
    for i, (df, label, color, ls, marker) in enumerate(
        ...
        if "WT" in label:
            ...
            ## plot everything but than negative beta
            ls = "dashed"
            plot_on_ax(
                ax_cl,
                -df[aoa_col],
    ...
    for i, ax in enumerate([ax_cl, ax_cd, ax_cs, ax_cmx, ax_cmy, ax_cmz]):
        ...
        if i == 2:
            ...
            y_min_allowed, y_max_allowed = -0.25, 0.25
        elif i == 6:
    ...
    file_name = f"WESC_beta_sweep_{beta_dist}"
